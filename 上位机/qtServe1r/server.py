...
class WindowM(QMainWindow, Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        self.dial_21.valueChanged.connect(self.dial_21_tri_cb)
        self.dial_22.valueChanged.connect(self.dial_22_tri_cb)
        ...
        self.checkBox.stateChanged.connect(self.checkBox_clk)
    ...
    def dial_21_tri_cb(self):
        self.lcdNumber_6.setProperty("value", self.dial_21.value())
        self.countDownWait = self.dial_21.value()
        
    
    def dial_22_tri_cb(self):
        self.lcdNumber_7.setProperty("value", self.dial_22.value())
        self.fixLength = self.dial_22.value()
        
    
    def checkBox_clk(self, val):
        self.label_2.setVisible(val == 2)
        self.label_4.setVisible(val == 2)
        self.lcdNumber_2.setVisible(val == 2)
    ...
    def runTimer_cb(self):
        ...
        if(self.countDownWait == 0):
            ...
            if(self.pastTime >= (27 + self.countDownWait)):
                print("TimeOut")
                if not self.lengFin:
                    self.lengFin = True
            ...
            if (self.lengFin and self.argFin):
                self.startRun = False
                self.runTimer.stop()
                self.label_27.setText(u"测量完成")
                try:
                    os.system('mpg123 msg.wav')
                except:
                    pass
    ...
    def calc_timer_cb(self):
        if len(self.cam0_info_list) > self.thSyn1 and len(self.cam1_info_list) > self.thSyn2:
            ...
            #print( cam0_dat['x_max_len'], cam1_dat['x_max_len'] )
            
            
            if(cam0_dat['x_max_len'] < 10):
                cam0_dat['line_length'] = cam1_dat['line_length']
            
            if(cam1_dat['x_max_len'] < 10):
                cam1_dat['line_length'] = cam0_dat['line_length']
                
            
            if (abs(cam0_dat['line_length'] - cam1_dat['line_length']) < 10) and (cam0_dat['x_max_len'] <= 400) and (cam1_dat['x_max_len'] <= 400):
